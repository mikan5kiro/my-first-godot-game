...
const TOMATO_EGG_RECIPE: RecipeData = preload("res://resources/recipes/tomato_egg.tres")
		...
		[
			{
				"id": CHOICE_YES,
				"label": "要",
				"confirm_sfx": STOVE_IGNITION_SFX,
				"confirm_sfx_duration": ignition_sfx_duration,
			},
...
func _run_cooking_sequence(player_interactor: PlayerInteractor) -> void:
	if not GameState.has_recipe_ingredients(TOMATO_EGG_RECIPE):
...
func _perform_cooking_on_black() -> void:
	GameState.cook_recipe(TOMATO_EGG_RECIPE, false)
...
func _on_cooking_fade_out_start(player_interactor: PlayerInteractor) -> void:
	...
	player_interactor.show_text("获得 %s。" % TOMATO_EGG_RECIPE.get_result_name())
